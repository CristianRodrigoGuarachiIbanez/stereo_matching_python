...
cdef class ImageAnalysisManager:
    cdef:
        ...
        freq * MAX
        freq *MIN
        int dimN,dimU,dimD,dimT
    def __cinit__(self, uchar[:,:,:,:] image):
        self.dimN = image.shape[0] #7
        self.dimU = image.shape[1] #8
        self.dimD = image.shape[2] #120
        self.dimT = image.shape[3] #160
        ...
        for i in range(self.dimN):
            for j in range(self.dimU):
                for k in range(self.dimD):
                    for n in range(self.dimT):
                        curr.key = image[i,j,k,n]
                        curr.value = 1
                        if not (self.findKey(self.frequency, curr.key)):
                            self.frequency.push_back(curr)
                            #self.frequency.push_back(freq())
                            #self.frequency[-1].key = curr.key
                            #self.frequency[-1].value = curr.value
                        else:
                            self.addValues(curr)

                        if(k+1==self.dimD and n+1==self.dimT):
